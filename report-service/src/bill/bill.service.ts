...
export class BillService {
  ...
  async streamBillsToS3WithProgress(
    onProgress: (progress: number) => void,
  ): Promise<{ fileUrl: string; downloadURL: string }> {
    console.log('Starting to stream bills to Bucket with progress updates...');
    ...
    try {
      ...
      console.log('Data successfully uploaded to S3 with progress updates');
      return { fileUrl, downloadURL };
    } catch (error) {
      console.error(
        'Error during streaming or uploading to S3 with progress:',
        error,
      );
      throw new Error('Failed to stream bills to S3 with progress');
    } finally {
      if (client) {
        try {
          client.release();
